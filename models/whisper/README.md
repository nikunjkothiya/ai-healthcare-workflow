...
```
Host path:      ./models/whisper/          →  Container path: /models/
```

So when `.env` says `WHISPER_MODEL_PATH=/models/ggml-small.en.bin`, it reads from **this folder**.
...
2. Update `WHISPER_MODEL_PATH` in `.env` (e.g., `/models/ggml-medium.en.bin`)
